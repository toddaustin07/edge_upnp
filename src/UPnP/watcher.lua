  ...
  for usn, devicetable in pairs(watchtable) do
    ...
    if devicetable.online then
      ...
      if devicetable.expiration < t then
        ...
        log.info (string.format('[upnp] Device %s seems to have gone offline; last exp: %s', devicetable.usn, os.date("%I:%M:%S",devicetable.expiration)))
        ...
        if devicetable._retries <= 2 then
          ...
          log.debug (string.format('[upnp] Sending unicast discovery (retry #%d) to %s:%s', devicetable._retries, devicetable.ip, port))
          ...
          if devicetable._retries <= 5 then
            log.debug (string.format('[upnp] Sending multicast discovery (retry #%d)', devicetable._retries-2))
      ...
      if string.find(id, devicetable.uuid, nil, 'plaintext') then     -- any usn containing the target device UUID is good enough
        ...
        if (headers["nts"] == 'ssdp:alive') or headers["st"] then      -- don't care what's in ST
          ...
          if headers["cache-control"] ~= nil then
            ...
            --devicetable.expiration = t + 30         -- for testing to force expiration
